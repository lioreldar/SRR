...
def print_srr(dataset, model_rect):
    ...
    for idx in range(GMM_Params.number_of_clusters):
        ax.add_patch(Rectangle(model_rect[idx].anchor,
                               width=2 * model_rect[idx].width,
                               height=2 * model_rect[idx].height,
                               angle=model_rect[idx].angle * 180 / np.pi,
                               fill=False,
                               linewidth=2))
    plt.show()
...
def generate_bounding_rect(covariance, mean):
    ...
    width = GMM_Params.side_length_factor * np.sqrt(eigenvalues[1])
    height = GMM_Params.side_length_factor * np.sqrt(eigenvalues[0])
